...
is confirmed unavailable, do not begin BOSS screening. First send the program
administrator a Feishu risk alert with:
...
This risk-alert script calls Feishu OpenAPI directly and does not require GPT.
`--fallback-provider deepseek|minimax` is optional only for a short supplemental
message when those credentials are configured and working; if the fallback model
fails, keep the fixed Feishu alert path.
...
These rules are permanent for all scheduled and manual XiaoZhao 2.2 runs.

Before any BOSS operation, run the startup checks first:

- Confirm the current local time is inside the configured run window and the
  task is not paused.
- Check whether a previous XiaoZhao/BOSS automation run is still active. If a
  stale task, unfinished child process, locked database writer, or unfinished
  browser automation state is found, do not start screening until it is safely
  resolved or recorded as a blocker.
- Check SQLite availability and integrity for
  `.zhipin-copilot/recruitment.sqlite3`.
- Run the SQLite `health-check` before scheduled BOSS work and before scheduled
  reports when possible. Treat integrity failures, missing required tables, or
  stale/missing heartbeat results as blockers or report warnings according to
  the current step.
- Check that the required JD library exists and the BOSS page is already logged
  in and healthy before candidate processing begins.
- Record a startup run event. Routine startup notice is the ordinary Feishu
  group message `现在开始工作`.

At the end of every run, finish the current candidate first if one is already
open, then run the shutdown checks:

- Write all candidate, match, forward, processed-candidate, online-resume-view,
  run-event, and report-precheck data needed for daily/weekly reporting.
- Produce or refresh the current day's aggregate report source data in SQLite
  before ending the task, so the next scheduled daily or weekly report can send
  on time without needing to reopen BOSS.
- A separate daily report-prep automation must run every day at 23:00. It
  reads/writes SQLite only, does not operate BOSS, and stores the current day's
  daily report body in `reports` with `sent_status=PREPARED`.
- A separate weekly report-prep automation must run every Sunday at 23:30. It
  reads/writes SQLite only, does not operate BOSS, and stores the current
  Monday-Sunday weekly report body in `reports` with `sent_status=PREPARED`.
- Check for residual XiaoZhao/BOSS automation processes, child commands,
  pending browser automation work, SQLite WAL/lock problems, or unfinished task
  state that could affect the next day's run.
- Stop only processes that clearly belong to the current XiaoZhao automation
  run. Never kill unrelated browser, system, Codex, Hermes, or user processes.
- If any residual task cannot be safely stopped, record it as a blocker and
  notify according to the fault rules instead of silently ending.
- Record a shutdown run event. Routine ending notice is the ordinary Feishu
  group message `到点了，下班！`.

For recoverable infrastructure faults (Feishu routine notice send failure,
temporary control-channel errors, stale locks, or residual
automation state), do not treat the first failure as a final stop. First run a
bounded self-repair/retry path: record the fault, refresh credentials if
applicable, clear only XiaoZhao-owned stale state, retry the
failed infrastructure step, and resume/restart the flow when the BOSS page
remains healthy. Non-recoverable BOSS safety blockers still fail closed: login
loss, CAPTCHA, slider, SMS/security verification, account abnormality, ambiguous
UI, or any risky state-changing click that cannot be verified must stop BOSS
operations and notify according to the fault rules.

The next day's task must always begin from startup checks. Never assume the
...
and `created_at`. Do not persist full online-resume section records during
normal screening; read the live resume only as needed for hard-requirement
judgment.
Candidate dedupe should prefer the lightweight fingerprint made from name, age,
and education when those fields are visible, falling back to name-only matching
only when age and education are unavailable.
For common names or anonymized names such as `王先生`, record the current
screening job plus age, education, expectation/target role, city, salary, and
years of experience whenever visible so second-pass dedupe can distinguish
different people.
...
1a. Before entering candidate screening, run `sqlite_store.py seed-jd-hard-gates`
    once. This converts the user-provided JD hard requirements into SQLite
    `job_hard_gates` rows so age/education/list prefilters do not repeatedly
    re-parse JD text.
1b. At workflow startup or recovery, at most one BOSS page refresh/reload is
...
3a. After selecting a job, run `sqlite_store.py get-jd-hard-gates --job-ref "<岗位>"`
    once and use the returned SQLite hard gates for the current job until the
    job changes. Permanent screening rule: the list page may judge only
    candidate age and education. Product, region, years of experience,
    language, business type, tool/platform, and every other hard requirement
    must be judged only after opening and reading the online resume.
4. In the `最新` list page, read all visible candidate rows before
   clicking anyone. Extract each visible row's name, age, education,
   expectation/target role, city, salary, and years of experience whenever
   visible.
5. Run a single batch SQLite duplicate plus age/education-only prefilter for
   the visible rows. Pass the current automation `run_id` to `batch-list-prefilter`
   so `screening_run_seen_candidates` skips candidates already seen in the same
   run/list-scroll loop. The duplicate check uses the name/age/education
   fingerprint when available. After list screening, first write the current
   batch's minimal candidate information to SQLite, then build an `open_resume`
   queue before clicking anyone.
6. If list-visible age or education clearly fails the selected JD hard gate, do
   not open the online resume; write 岗位/姓名/年龄/学历/求职期望/目标岗位/城市/薪资/工作年限 to SQLite and count
...
10. Judge whether the candidate is suitable by analyzing the complete live
    ...
    evidence means the hard requirement is not satisfied. Follow `decision_plan`
    only at this online-resume stage for
...
- Use only the already visible BOSS browser/session. Do not open or close
  browser windows, tabs, or apps during candidate screening.
- Do not change browser zoom, page zoom, display scaling, or viewport size for
  speed. Browser scaling is forbidden during BOSS screening.
